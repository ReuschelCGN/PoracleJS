	...
	async handle(obj) {
		...
		try {
			...
			if (this.config.general.pmsfMapURL) {
				data.pmsfMapUrl = `${this.config.general.pmsfMapURL}${!this.config.general.pmsfMapURL.endsWith('/') ? '/' : ''}?lat=${data.latitude}&lon=${data.longitude}&zoom=18&encId=${data.encounter_id}`
			}
