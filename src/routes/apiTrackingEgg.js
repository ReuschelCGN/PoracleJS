		...
		try {
			...
			if ((alreadyPresent.length + updates.length + insert.length) > 50) {
				...
				for (const egg of alreadyPresent) {
					message = message.concat(translator.translate('Unchanged: '), await trackedCommand.eggRowText(fastify.config, translator, fastify.GameData, egg, fastify.scannerQuery), '\n')
				}
				for (const egg of updates) {
					message = message.concat(translator.translate('Updated: '), await trackedCommand.eggRowText(fastify.config, translator, fastify.GameData, egg, fastify.scannerQuery), '\n')
				}
				for (const egg of insert) {
					message = message.concat(translator.translate('New: '), await trackedCommand.eggRowText(fastify.config, translator, fastify.GameData, egg, fastify.scannerQuery), '\n')
				}
			}
