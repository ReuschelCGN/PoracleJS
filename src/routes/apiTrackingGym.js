		...
		try {
			...
			if ((alreadyPresent.length + updates.length + insert.length) > 50) {
				...
				for (const gym of alreadyPresent) {
					message = message.concat(translator.translate('Unchanged: '), await trackedCommand.gymRowText(fastify.config, translator, fastify.GameData, gym, fastify.scannerQuery), '\n')
				}
				for (const gym of updates) {
					message = message.concat(translator.translate('Updated: '), await trackedCommand.gymRowText(fastify.config, translator, fastify.GameData, gym, fastify.scannerQuery), '\n')
				}
				for (const gym of insert) {
					message = message.concat(translator.translate('New: '), await trackedCommand.gymRowText(fastify.config, translator, fastify.GameData, gym, fastify.scannerQuery), '\n')
				}
			}
