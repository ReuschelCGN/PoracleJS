		...
		try {
			...
			if ((alreadyPresent.length + updates.length + insert.length) > 50) {
				...
				for (const raid of alreadyPresent) {
					message = message.concat(translator.translate('Unchanged: '), await trackedCommand.raidRowText(fastify.config, translator, fastify.GameData, raid, fastify.scannerQuery), '\n')
				}
				for (const raid of updates) {
					message = message.concat(translator.translate('Updated: '), await trackedCommand.raidRowText(fastify.config, translator, fastify.GameData, raid, fastify.scannerQuery), '\n')
				}
				for (const raid of insert) {
					message = message.concat(translator.translate('New: '), await trackedCommand.raidRowText(fastify.config, translator, fastify.GameData, raid, fastify.scannerQuery), '\n')
				}
			}
