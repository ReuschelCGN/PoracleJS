		...
		try {
			...
			if ((alreadyPresent.length + updates.length + insert.length) > 50) {
				...
				for (const lure of alreadyPresent) {
					message = message.concat(translator.translate('Unchanged: '), await trackedCommand.maxbattleRowText(fastify.config, translator, fastify.GameData, lure, fastify.scannerQuery), '\n')
				}
				for (const lure of updates) {
					message = message.concat(translator.translate('Updated: '), await trackedCommand.maxbattleRowText(fastify.config, translator, fastify.GameData, lure, fastify.scannerQuery), '\n')
				}
				for (const lure of insert) {
					message = message.concat(translator.translate('New: '), await trackedCommand.maxbattleRowText(fastify.config, translator, fastify.GameData, lure, fastify.scannerQuery), '\n')
				}
			}
