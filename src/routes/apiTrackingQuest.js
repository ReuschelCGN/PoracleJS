	...
	fastify.post('/api/tracking/quest/:id', options, async (req) => {
		...
		try {
			...
			if ((alreadyPresent.length + updates.length + insert.length) > 50) {
				...
				for (const quest of alreadyPresent) {
					message = message.concat(translator.translate('Unchanged: '), trackedCommand.questRowText(fastify.config, translator, fastify.GameData, quest), '\n')
				}
				for (const quest of updates) {
					message = message.concat(translator.translate('Updated: '), trackedCommand.questRowText(fastify.config, translator, fastify.GameData, quest), '\n')
				}
				for (const quest of insert) {
					message = message.concat(translator.translate('New: '), trackedCommand.questRowText(fastify.config, translator, fastify.GameData, quest), '\n')
				}
			}
