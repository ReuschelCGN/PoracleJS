	...
	fastify.post('/api/tracking/pokemon/:id', options, async (req) => {
		...
		try {
			...
			if ((alreadyPresent.length + updates.length + insert.length) > 50) {
				...
				for (const pokemon of alreadyPresent) {
					message = message.concat(translator.translate('Unchanged: '), trackedCommand.monsterRowText(fastify.config, translator, fastify.GameData, pokemon), '\n')
				}
				for (const pokemon of updates) {
					message = message.concat(translator.translate('Updated: '), trackedCommand.monsterRowText(fastify.config, translator, fastify.GameData, pokemon), '\n')
				}
				for (const pokemon of insert) {
					message = message.concat(translator.translate('New: '), trackedCommand.monsterRowText(fastify.config, translator, fastify.GameData, pokemon), '\n')
				}
			}
