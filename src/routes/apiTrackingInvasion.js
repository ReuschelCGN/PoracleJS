	...
	fastify.post('/api/tracking/invasion/:id', options, async (req) => {
		...
		try {
			...
			if ((alreadyPresent.length + updates.length + insert.length) > 50) {
				...
				for (const invasion of alreadyPresent) {
					message = message.concat(translator.translate('Unchanged: '), trackedCommand.invasionRowText(fastify.config, translator, fastify.GameData, invasion), '\n')
				}
				for (const invasion of updates) {
					message = message.concat(translator.translate('Updated: '), trackedCommand.invasionRowText(fastify.config, translator, fastify.GameData, invasion), '\n')
				}
				for (const invasion of insert) {
					message = message.concat(translator.translate('New: '), trackedCommand.invasionRowText(fastify.config, translator, fastify.GameData, invasion), '\n')
				}
			}
